...
import com.orileo.attendance.wrapper.entity.mysql.astra.PermanentContractAttendance;
...
public interface PermanentContractAttendanceRepo extends JpaRepository<PermanentContractAttendance, Long> {
    ...
    @Query("SELECT p FROM PermanentContractAttendance p where p.employeeCode = :employeeId and p.markedOn =:startDate")
    PermanentContractAttendance getEmployeeAttendanceByEmployeeCode(@Param("startDate") Date startDate, @Param("employeeId") String employeeId);
